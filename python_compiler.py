...
class IndexAssignmentNode(ASTNode):
    def __init__(self, target, index_expr, value_expr, line=0):
        # target puede ser un string (nombre simple) o un IndexNode (acceso anidado)
        self.target = target
...
class GlobalNode(ASTNode):
    def __init__(self, variables, line=0):
        self.variables = variables
        self.line = line

class TryNode(ASTNode):
    def __init__(self, try_block, except_blocks, line=0):
...
class Parser:
    ...
    def parse_statement(self):
        ...
        elif token_type == TokenType.GLOBAL:
            return self.parse_global()
        elif token_type == TokenType.TRY:
            return self.parse_try()
        elif token_type == TokenType.DEL:
    ...
    def parse_list_assignment(self):
        ...
        # Manejar múltiples índices encadenados: dict[key1][key2] = value
        indices = []
        ...
        # Si hay múltiples índices, crear accesos anidados
        if len(indices) == 1:
            return IndexAssignmentNode(identifier, indices[0], value_expr, line)
        else:
            # Para múltiples índices: base[idx1][idx2] = value
            # Crear un IndexNode anidado para el lado izquierdo
            base = IdentifierNode(identifier, line)
            for idx in indices[:-1]:
                base = IndexNode(base, idx, line)
            return IndexAssignmentNode(base, indices[-1], value_expr, line)
    ...
    def parse_function(self):
        ...
        return FunctionNode(name, params, body, line)
    ...
    def parse_global(self):
        ...
        while self.current_token.type == TokenType.COMMA:
            self.advance()
            variables.append(self.expect(TokenType.IDENTIFIER).value)
        return GlobalNode(variables, line)
    ...
    def parse_try(self):
        ...
        while self.current_token.type == TokenType.EXCEPT:
            ...
            except_blocks.append((exception_type, except_block))
        
        return TryNode(try_block, except_blocks, line)
    
    def parse_del(self):
        line = self.current_token.line
        self.expect(TokenType.DEL)
        target = self.parse_expression()
        return DelNode(target, line)
    ...
    def parse_factor(self):
        ...
        if token.type == TokenType.NUMBER:
            ...
            return DictionaryNode(items, token.line)
        elif token.type == TokenType.LPAREN:
            self.advance()
            expr = self.parse_expression()
            self.expect(TokenType.RPAREN)
            return expr
        elif token.type in (TokenType.RANGE, TokenType.LEN, TokenType.INPUT, TokenType.INT, TokenType.FLOAT, TokenType.STR):
            func_name = token.value
            self.advance()
            self.expect(TokenType.LPAREN)
            args = []
